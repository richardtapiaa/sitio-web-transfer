...
const CTabPanel = React.forwardRef((_a, ref) => {
    var { children, className, itemKey, onHide, onShow, transition = true, visible } = _a, rest = tslib_es6.__rest(_a, ["children", "className", "itemKey", "onHide", "onShow", "transition", "visible"]);
    const { _activeItemKey, id } = React.useContext(CTabsContext.CTabsContext);
    const tabPaneRef = React.useRef(null);
    const forkedRef = useForkedRef.useForkedRef(ref, tabPaneRef);
    const [_visible, setVisible] = React.useState(visible || _activeItemKey === itemKey);
    React.useEffect(() => {
    ...
    return (React.createElement(Transition.default, { in: _visible, nodeRef: tabPaneRef, onEnter: onShow, onExit: onHide, timeout: tabPaneRef.current ? getTransitionDurationFromElement.default(tabPaneRef.current) : 0 }, (state) => (React.createElement("div", Object.assign({ className: index.default('tab-pane', {
            active: _visible,
            fade: transition,
            show: state === 'entered',
