...
const CMultiSelect = React.forwardRef((_a, ref) => {
    ...
    const filteredOptions = React.useMemo(() => utils.flattenOptionsArray(utils.isExternalSearch(search)
        ? [...options, ...utils.filterOptionsList(searchValue, userOptions)]
        : utils.filterOptionsList(searchValue, [...options, ...userOptions]), true), [options, searchValue, userOptions]);
    const flattenedOptions = React.useMemo(() => utils.flattenOptionsArray(options).map((option) => {
    ...
    const handleSearchKeyDown = (event) => {
        ...
        if (event.key === 'ArrowDown' &&
            ...
            const items = utils.getOptionsList(dropdownMenuElement.current);
    ...
    const handleSelectAll = () => {
        setSelected(utils.selectOptions(multiple, [...flattenedOptions.filter((option) => !option.disabled), ...userOptions], selected));
    };
    const handleDeselectAll = () => {
        setSelected(selected.filter((option) => option.disabled));
    };
    return (React.createElement(CFormControlWrapper.CFormControlWrapper, { describedby: rest['aria-describedby'], feedback: feedback, feedbackInvalid: feedbackInvalid, feedbackValid: feedbackValid, id: id !== null && id !== void 0 ? id : uniqueId, invalid: invalid, label: label, text: text, tooltipFeedback: tooltipFeedback, valid: valid },
        React.createElement(CMultiSelectNativeSelect.CMultiSelectNativeSelect, { id: id !== null && id !== void 0 ? id : uniqueId, multiple: multiple, name: name !== null && name !== void 0 ? name : uniqueId, options: selected, required: required, value: multiple
        ...
        React.createElement("div", { className: index.default('form-multi-select', {
            ...
            React.createElement("div", Object.assign({ className: "form-multi-select-input-group" }, (!search && !disabled && { tabIndex: 0 }), { onClick: () => !disabled && openDropdown(), onKeyDown: handleTogglerKeyDown, role: "combobox", "aria-haspopup": "listbox", "aria-expanded": isOpen }, (portal && { 'aria-owns': `multiselect-listbox-${uniqueId}` }), { ref: dropdownRefElement }),
                React.createElement(CMultiSelectSelection.CMultiSelectSelection, { disabled: disabled, multiple: multiple, onRemove: (option) => !disabled && handleOnOptionClick(option), placeholder: placeholder, search: search, selected: selected, selectionType: selectionType, selectionTypeCounterText: selectionTypeCounterText },
            ...
            React.createElement(CConditionalPortal.CConditionalPortal, { container: container, portal: portal },
                React.createElement("div", { className: index.default('form-multi-select-dropdown', {
                    ...
                    React.createElement(CMultiSelectOptions.CMultiSelectOptions, { loading: loading, onOptionOnClick: (option) => !disabled && handleOnOptionClick(option), options: filteredOptions.length === 0 && allowCreateOptions
                            ? userOption || []
...
CMultiSelect.propTypes = Object.assign({ allowCreateOptions: PropTypes.bool, ariaCleanerLabel: PropTypes.string, ariaIndicatorLabel: PropTypes.string, className: PropTypes.string, cleaner: PropTypes.bool, clearSearchOnSelect: PropTypes.bool, container: PropTypes.any, disabled: PropTypes.bool, loading: PropTypes.bool, multiple: PropTypes.bool, name: PropTypes.string, onChange: PropTypes.func, onFilterChange: PropTypes.func, onHide: PropTypes.func, onShow: PropTypes.func, options: PropTypes.array.isRequired, optionsMaxHeight: PropTypes.oneOfType([PropTypes.number, PropTypes.string]), optionsStyle: PropTypes.oneOf(['checkbox', 'text']), optionsTemplate: PropTypes.func, optionsGroupsTemplate: PropTypes.func, placeholder: PropTypes.string, portal: PropTypes.bool, required: PropTypes.bool, resetSelectionOnOptionsChange: PropTypes.bool, search: PropTypes.oneOfType([
    ...
    ]), virtualScroller: PropTypes.bool, visible: PropTypes.bool, visibleItems: PropTypes.number }, CFormControlWrapper.CFormControlWrapper.propTypes);
