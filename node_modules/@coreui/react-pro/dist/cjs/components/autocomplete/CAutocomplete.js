...
const CAutocomplete = React.forwardRef((_a, ref) => {
    var { allowOnlyDefinedOptions = false, className, cleaner = false, clearSearchOnSelect = true, container, disabled, feedback, feedbackInvalid, feedbackValid, highlightOptionsOnSearch = false, id, indicator, invalid, label, loading, name, onChange, onHide, onInput, onShow, options, optionsMaxHeight = 'auto', optionsTemplate, optionsGroupsTemplate, placeholder, portal = false, readOnly, required, resetSelectionOnOptionsChange = false, search, searchNoResultsLabel = false, showHints = false, size, text, tooltipFeedback, valid, value, virtualScroller, visible, visibleItems = 10 } = _a, rest = tslib_es6.__rest(_a, ["allowOnlyDefinedOptions", "className", "cleaner", "clearSearchOnSelect", "container", "disabled", "feedback", "feedbackInvalid", "feedbackValid", "highlightOptionsOnSearch", "id", "indicator", "invalid", "label", "loading", "name", "onChange", "onHide", "onInput", "onShow", "options", "optionsMaxHeight", "optionsTemplate", "optionsGroupsTemplate", "placeholder", "portal", "readOnly", "required", "resetSelectionOnOptionsChange", "search", "searchNoResultsLabel", "showHints", "size", "text", "tooltipFeedback", "valid", "value", "virtualScroller", "visible", "visibleItems"]);
    const autoCompleteRef = React.useRef(null);
    const autoCompleteForkedRef = useForkedRef.useForkedRef(ref, autoCompleteRef);
    const dropdownRef = React.useRef(null);
    const togglerRef = React.useRef(null);
    const inputRef = React.useRef(null);
    const inputHintRef = React.useRef(null);
    ...
    const filteredOptions = React.useMemo(() => (utils.isExternalSearch(search) ? options : utils.filterOptions(options, searchValue)), [options, searchValue, search]);
    ...
    React.useEffect(() => {
        ...
        if (value && typeof value === 'number') {
            const foundOption = utils.getFirstOptionByValue(value, options);
            if (foundOption) {
                handleSelect(foundOption);
            }
            return;
        }
        const _selected = utils.flattenOptionsArray(filteredOptions).find((option) => typeof option !== 'string' && option.selected === true);
        if (_selected) {
    ...
    React.useEffect(() => {
        ...
        const findOption = searchValue.length > 0
            ? filteredOptions.find((option) => utils.getOptionLabel(option).toLowerCase().startsWith(searchValue.toLowerCase()))
            : undefined;
        setHint(findOption);
    ...
    const handleInputBlur = (event) => {
        ...
        const exactMatches = utils.flattenOptionsArray(options).filter((option) => utils.getOptionLabel(option).toLowerCase() === inputValueLower);
    ...
    const handleInputKeyDown = (event) => {
        ...
        if (event.key === 'Enter') {
            ...
            const foundOptions = utils.getFirstOptionByLabel(input.value, filteredOptions);
            if (foundOptions) {
    ...
    return (React.createElement(CFormControlWrapper.CFormControlWrapper, { describedby: rest['aria-describedby'], feedback: feedback, feedbackInvalid: feedbackInvalid, feedbackValid: feedbackValid, id: id || `autocomplete-${uniqueId}`, invalid: invalid, label: label, text: text, tooltipFeedback: tooltipFeedback, valid: valid },
        React.createElement("div", { className: index.default('autocomplete', {
            ...
            React.createElement("div", { className: "autocomplete-input-group", onClick: () => handleDropdownShow(), ref: togglerRef },
                showHints && searchValue !== '' && (React.createElement("input", { className: "autocomplete-input autocomplete-input-hint", id: id || `autocomplete-hint-${uniqueId}`, autoComplete: "off", readOnly: true, tabIndex: -1, "aria-hidden": "true", value: hint ? `${searchValue}${utils.getOptionLabel(hint).slice(searchValue.length)}` : '', ref: inputHintRef })),
                React.createElement("input", Object.assign({ type: "text", className: "autocomplete-input", disabled: disabled, id: id || `autocomplete-${uniqueId}`, name: name || `autocomplete-${uniqueId}`, onBlur: handleInputBlur, onChange: handleInputChange, onKeyDown: handleInputKeyDown, placeholder: placeholder, autoComplete: "off", required: required, "aria-autocomplete": "list", "aria-expanded": _visible, "aria-haspopup": "listbox" }, (portal && { 'aria-owns': `autocomplete-listbox-${uniqueId}` }), { readOnly: readOnly, role: "combobox", ref: inputRef })),
                (cleaner || indicator) && (React.createElement("div", { className: "autocomplete-buttons" },
                    ...
                    indicator && (React.createElement("button", { type: "button", className: "autocomplete-indicator", disabled: !(searchNoResultsLabel || filteredOptions.length > 0) &&
                            utils.isExternalSearch(search), onClick: (event) => {
                            event.preventDefault();
                            event.stopPropagation();
                            if (_visible) {
                                handleDropdownHide();
